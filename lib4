...
function SaveThemeCfg()
	if not LoadedThemeFile then return; end

	local Data = {}

	for key, obj in pairs(CTC) do
		local value = obj.Value
		if typeof(value) == "Color3" then
			Data[key] = PackColor(value)
		end
	end
...
function RequestSaveTheme()
	...
	task.delay(1, function()
		...
		local hash = HttpService:JSONEncode(CTC)
...
function LoadThemeCfg(Config)
	...
	for key, obj in pairs(CTC) do
		local packed = Data[key]
		if packed and typeof(obj.Value) == "Color3" then
			obj.Value = UnpackColor(packed)
		end
	end
...
local THEME_DELAY = 1 -- 40ms (25 updates por segundo)
local LastThemeUpdate = 0
local PendingThemeUpdate = false

local ThemeConnections = {}

local LastApplyTime = {}
local APPLY_COOLDOWN = 0.5 -- 80ms (≈12.5 updates por segundo)
...
local function ApplyTheme(Type)
	local now = os.clock()
	local last = LastApplyTime[Type] or 0

	if now - last < APPLY_COOLDOWN then
		return -- ainda está em cooldown, pula
	end

	LastApplyTime[Type] = now
...
--[[
local function MarkThemeDirty(Type)
	DirtyThemes[Type] = true

	local now = os.clock()

	if now - LastThemeUpdate >= THEME_DELAY then
		LastThemeUpdate = now
	...
	RunService.Heartbeat:Once(function()
		...
		for Type in pairs(DirtyThemes) do
			ApplyTheme(Type)
		end

		table.clear(DirtyThemes)
	end)
end

task.spawn(function()
	while true do
		task.wait(THEME_DELAY)

		if PendingThemeUpdate then
			PendingThemeUpdate = false
			LastThemeUpdate = os.clock()
...
local function EnsureThemeConnection(Type)
	if ThemeConnections[Type] then return end

	ThemeConnections[Type] = CTC[Type].Changed:Connect(function(v)
		OrionLib.Themes[OrionLib.SelectedTheme][Type] = v
		MarkThemeDirty(Type)
		RequestSaveTheme()
	end)
end]

-- 🔥 SUA FUNÇÃO REESCRITA
local function AddThemeObject(Object, Type)
	...
	if not prop or Object[prop] == nil then
		return Object
	end
	...
	-- Garante só 1 Changed por Type
	EnsureThemeConnection(Type)
...
local DirtyThemes = {}
local ThemeFlushScheduled = false

local function MarkThemeDirty(Type)
	DirtyThemes[Type] = true

	if ThemeFlushScheduled then return end
	ThemeFlushScheduled = true

	task.delay(0.2, function() -- 40ms ≈ 25fps
		ThemeFlushScheduled = false

		-- sincroniza com frame
...
local function AddThemeObject(Object, Type)
	...
	local p = ReturnProperty(Object)

	if p and Object[p] ~= nil then
		table.insert(OrionLib.ThemeObjects[Type], Object)
		Object[p] = OrionLib.Themes[OrionLib.SelectedTheme][Type]
		
		AddConnection(CTC[Type].Changed, function(v)
			RunService.Heartbeat:Wait()
			OrionLib.Themes[OrionLib.SelectedTheme][Type] = v

			if not table.find(OrionLib.ThemeObjects[Type], Object) then return end
...
function OrionLib:MakeWindow(WindowConfig)
	...
	AddConnection(CTC["Main"].Changed, CSBC)
	...
	mainWindowStroke = Instance.new("UIStroke", MainWindow)
	...
	--[[
	AddConnection(CTC["Stroke"].Changed, function(v) 
		mainWindowGradient.Color = createGradientFromColor(v, 0.5)
	end)
	]]--
    ...
    local MAX_SIZE = Vector2.new(900, 600)
...
function ElementFunction:AddPlayersDropdown(DropdownConfig)
	...
	if DropdownConfig.MultipleSelection and DropdownConfig.Default ~= "" then
		if type(DropdownConfig.Default) == "table" then
			for _, default in pairs(DropdownConfig.Default) do
				table.insert(SelectedValues, default)
			end
		else
			local defaults = string.split(DropdownConfig.Default, ",")
			for _, default in pairs(defaults) do
				table.insert(SelectedValues, string.gsub(default, "^%s+|%s+$", "")) -- Remove espaços
			end
		end
	end
	...
	return Dropdown
			...
			function ElementFunction:AddColorpicker(ColorpickerConfig)
				...
				local ColorpickerBox = SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 4), {
					Size = UDim2.new(0, 24, 0, 24),
					Position = UDim2.new(1, -12, 0.5, 0),
					AnchorPoint = Vector2.new(1, 0.5)
				}), {
    ...
    function TabFunction:AddConfigTab()
		...
		local main_color = config_tab:AddSection({
			Name = "Smart Theme Custom"
		})

		local multi_color = config_tab:AddSection({
			Name = "Multiple Theme Custom"
		})
		...
		-- Escurece cor (RGB)
		local function darkerColor(color: Color3, factor: number)
			factor = math.clamp(factor, 0, 1)
		...
		-- Clareia cor (HSV - funciona com cores saturadas)
		...
		-- Decide automaticamente se clareia ou escurece
		...
		function createGradientFromColor(baseColor, factor)
			local light = smartMidColor(baseColor, factor)
		...
		AddConnection(CTC["Stroke"].Changed, function(v)
			if not _G.RWMode then
				mainWindowGradient.Color = createGradientFromColor(v, 0.7)
			end
		end)

		windowstroke:AddToggle({
			Name = "Rainbow Mode",
			Default = false,
			Callback = function(Value)
				_G.RWMode = Value
				...
				if Value then
					...
					mainWindowGradient.Color = createGradientFromColor(CTC["Stroke"].Value, 0.7)
				end
			end,
		})
		...
		for t, v in pairs(CTC) do
            local c = multi_color:AddColorpicker({
                Name = t,
                Default = OrionLib.Themes[OrionLib.SelectedTheme][t],
                Callback = function(Value)
                    v.Value = Value
                end,
            })
		...
		select_theme:AddDropdown({
			Name = "Select Theme",
			Default = "Default",
			Options = {"Default", "Bliz_T"},
			Callback = function(Value)
				OrionLib.SelectedTheme = Value

				for t, v in pairs(CTC) do
					v.Value = OrionLib.Themes[OrionLib.SelectedTheme][t]
				end
			end    
		})

		main_color:AddColorpicker({
			Name = "Smart Theme",
			Default = OrionLib.Themes["Default"].Main,
			Callback = function(Value)
				local newTheme = OrionLib:GenTheme(Value)
				OrionLib.Themes.Custom = newTheme
				OrionLib.SelectedTheme = "Custom"

				CTC["Main"].Value = OrionLib.Themes.Custom["Main"]
				CTC["Second"].Value = OrionLib.Themes.Custom["Second"]
				CTC["Stroke"].Value = OrionLib.Themes.Custom["Stroke"]
				CTC["Divider"].Value = OrionLib.Themes.Custom["Divider"]
				CTC["Text"].Value = OrionLib.Themes.Custom["Text"]
				CTC["TextDark"].Value = OrionLib.Themes.Custom["TextDark"]
				--[[
				for t, v in pairs(CTC) do
					v.Value = OrionLib.Themes.Custom[t]
					picks[t]:Set(v.Value)
				end
				]]--
			end,
		...
		LoadThemeCfg(readfile(FILE_PATH))
    end
